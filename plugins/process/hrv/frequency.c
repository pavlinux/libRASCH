 ...
 *
 * $Header: /home/cvs_repos.bak/librasch/plugins/process/hrv/frequency.c,v 1.4 2003/08/22 08:52:08 rasch Exp $
 *--------------------------------------------------------------------------*/
...
make_equidistant(double *v, double *pos, long n, double distance, double **equi, long *n_equi)
{
	...
	*equi = malloc(sizeof(double) * (*n_equi));
	...
	for (l = 0; l < *n_equi; l++)
	{
		...
		fprintf(fp, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", (*equi)[l], curr_pos, before, pos_before, after, pos_after);
		curr_pos += distance;
	}
	fclose(fp);
	*n_equi = l;
...
calc_periodogram(double *v, long n, double samplefreq, double **spec, long *n_spec, char *window)
{
	...
	x = malloc(sizeof(complex) * n_fft);
	memset(x, 0, sizeof(complex) * n_fft);
	for (l = 0; l < n; l++)
		x[l].r = v[l];
...
int
calc_log_freq_smoothed_spec(double *spec, long n, double samplerate, double **smoothed, long *n_sm)
{
	double freq_step;
	int n_decades;
	int begin, end;
