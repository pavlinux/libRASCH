/*----------------------------------------------------------------------------
 * ctg_sonicaid.c  -  Handle cardiotocogram signals saved with the Oxford FetalCare system
 *
 * Description:
 * The plugin provides access to CTG's handled with the Oxford FetalCare system.
 * The information about the Mother and the recording date and time is stored in
 * a Access database-file. Therefore a text file similar to the one in the ctg-rasch
 * plugin will be used to handle this information (the file is called 'ctg_info.txt').
 *
...
get_data(struct info *i, int fast)
{
	...
	sprintf(file, "%s%c%s.%s%csonicaid.dat", i->dir, SEPARATOR, i->name, i->ext, SEPARATOR);
	fp = fopen(file, "rb");
	if (fp == NULL)
		goto error;

	fseek(fp, 0, SEEK_END);
	i->num_samples = (ftell(fp) - HEADER_SIZE) / sizeof(struct trace_data);
	fseek(fp, HEADER_SIZE, SEEK_SET);

	if (!fast)
	{
		i->raw_data = (struct trace_data *)malloc(sizeof(struct trace_data) * i->num_samples);
		if (fread(i->raw_data, sizeof(struct trace_data), i->num_samples, fp) != (size_t)(i->num_samples))
		{
			fclose(fp);
			goto error;
		}
	...
	fclose(fp);
	i->samplerate = 256.0 / 60.0;

	/* set informations about the trace data */
...
error:
	_ra_set_error(i->ra, RA_ERR_MEAS_CORRUPT, "plugin-ctg-sonicaid");
...
pl_get_info_idx(any_handle h, int info_type, int index, value_handle vh)
{
	...
	_ra_set_error(h, RA_ERR_UNKNOWN_INFO, "plugin-ctg-sonicaid");
...
get_info_id(any_handle h, int id, value_handle vh)
{
	...
	{
		_ra_set_error(h, RA_ERR_ERROR, "plugin-ctg-sonicaid");
		return -1;
	}
...
get_info_rec(struct info *i, int id, value_handle vh)
{
	...
	{
		_ra_set_error(i->ra, RA_ERR_OUT_OF_RANGE, "plugin-ctg-sonicaid");
		return -1;
	}
	...
	case RA_INFO_REC_GEN_DESC_C:
		ra_value_set_string_utf8(vh, "CTG signal");
		break;
	...
	case RA_INFO_REC_CH_IGNORE_VALUE_D:
		...
		break;
	default:
		_ra_set_error(i->ra, RA_ERR_UNKNOWN_INFO, "plugin-ctg-sonidaid");
		ret = -1;
		break;
	}
...
get_info_obj(struct info *i, int id, value_handle vh)
{
	...
	case RA_INFO_OBJ_PREG_WOMAN_GESTATION_DATE_C:
		...
		break;
	default:
		_ra_set_error(i->ra, RA_ERR_UNKNOWN_INFO, "plugin-ctg-sonicaid");
		ret = -1;
		break;
	}
...
pl_set_info(any_handle h, int id, value_handle vh)
{
	...
	case RA_INFO_REC_GEN_DATE_C:
		...
		break;		
	default:
		_ra_set_error(i->ra, RA_ERR_UNSUPPORTED, "plugin-ctg-sonicaid");
		ret = -1;
		break;
	}
...
read_raw(struct info * i, int ch, size_t start, size_t num, BYTE *buf)
{
	...
	if ((ch >= i->num_channels) || (start >= (size_t)(i->num_samples)))
	{
		_ra_set_error(i->ra, RA_ERR_OUT_OF_RANGE, "plugin-ctg-sonicaid");
		return 0;
	}
	if (i->raw_data == NULL)
	{
		_ra_set_error(i->ra, RA_ERR_DATA_NOT_AVAIL_FAST, "plugin-ctg-sonicaid");
		return 0;
	}
	
	num_use = num;
	if ((start + num_use) >= (size_t)(i->num_samples))
		num_use = i->num_samples - start;
	
	if (ch == 0)
	{
		for (l = start; l < (start + num_use); l++)
		{
			buf[l-start] = i->raw_data[l].fhr1;
			if (buf[l-start] >= 254)
				buf[l-start] = 0;
		}
	}
