 ...
 */
FILE *
_ra_fopen(const char *fn, const char *mode, int dir_based, char *fn_ok, int buf_len)
{
	FILE *fp = NULL;
	char *pos, *pos_save;
	int is_lower = -1;
 ...
 */
int
find_meas(const char *dir, const char *mask, const char *plugin_name, struct find_meas *f, int (*check_meas) (const char *))
{
	int n, i;
	char **files;
	...
	for (i = 0; i < n; i++)
	{
		...
		{
			f->names[f->num] = ra_alloc_mem(sizeof(char) * strlen(files[i]) + 1);
			strcpy(f->names[f->num], files[i]);

			f->plugins[f->num] = ra_alloc_mem(sizeof(char) * strlen(plugin_name) + 1);
 ...
 */
int
find_files(const char *dir, const char *mask, char ***f, int *num, long *size)
{
	int ret = -1;
	long s = 0;
...
#ifdef WIN32
int
find_files_win32(const char *dir, const char *mask, char ***f, int *num, long *size)
{
	char fm[MAX_PATH_RA];
	...
	*size = 0;
	...
	if (num_new > 0)
	{
		...
		if (*num > 0)
		{
			for (l = 0; l < *num; l++)
			{
				f_new[l] = ra_alloc_mem(sizeof(char) * (strlen((*f)[l])+1));
				strcpy(f_new[l], (*f)[l]);
				ra_free_mem((*f)[l]);
			}
			ra_free_mem(*f);
		...
		{
			f_new[l] = ra_alloc_mem(sizeof(char) * (strlen(f_temp[l-(*num)])+1));
...
#ifdef LINUX
int
find_files_linux(const char *dir, const char *mask, char ***f, int *num, long *size)
{
	char dir_locale[MAX_PATH_RA];
	DIR *d;
	struct dirent *de;
...
copy_move_file(const char *file, const char *dest_dir, int copy)
{
	...
	{
		if (MoveFileEx(file_locale, new_file_locale, MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED) == FALSE)
			ret = -1;
	}
#endif /* WIN32 */
...
#ifdef WIN32
	if (MoveFileEx(src_locale, dest_locale, MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED) == FALSE)
		ret = -1;
#endif /* WIN32 */
