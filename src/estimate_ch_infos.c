...
int isletter (char c);
int read_folder_ch_map (const char *folder, struct ch_map **map,
			int *num_entries);
int read_meas_ch_map (const char *filename, int is_dir_based,
		      struct ch_map **map, int *num_entries);
 ...
 */
LIBRAAPI int
ra_est_ch_type (ra_handle ra, const char *folder, const char *filename,
		int is_dir_based, const char *name,
		struct ra_est_ch_infos *inf)
{
  ...
  if (filename != NULL)
	...
	{
	  if ((RA_STRNICMP
	       (name_utf8, ch_map[i].ch_name,
		strlen (name_utf8)) == 0) && (ch_map[i].inf_idx >= 0))
	    {
  ...
  if (!found && lib)
	...
	{
	  if ((RA_STRNICMP
	       (name_utf8, lib->config.ch_map_system[i].ch_name,
		strlen (name_utf8)) == 0)
	      && (lib->config.ch_map_system[i].inf_idx >= 0))
	    {
	      memcpy (inf,
		      &(_inf
			[lib->config.ch_map_system[i].inf_idx]),
		      sizeof (_inf[0]));
	      found = 1;
	      break;
	    }
...
int
read_meas_ch_map (const char *filename, int is_dir_based, struct ch_map **map,
		  int *num_entries)
{
  int ret = 0;
  char fn[MAX_PATH_RA];
...
find_ch_map_entry (char *string, struct ch_map **map, int *num_entries)
{
  ...
  *map = realloc (*map, sizeof (struct ch_map) * (*num_entries));
  ...
  do
	...
	{
	  if ((RA_STRNICMP
	       (_inf[i].name_contains, p + 1,
		strlen (_inf[i].name_contains)) == 0)
	      && (((*map)[*num_entries - 1].ch_type == 0)
		  || ((*map)[*num_entries - 1].ch_type ==
		      _inf[i].parent_type)))
	    {
