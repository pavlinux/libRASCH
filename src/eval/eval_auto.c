...
LIBRAAPI int
ra_eval_save_result(eval_handle eh, class_handle clh, proc_handle proc, long res_set)
{
	int ret = 0;
	int i;
	eval_handle eh_use;
	...
	if (eh == NULL)
	{
		eh_use = ra_eval_add(pi->mh, "auto-eval", "evaluation created within libRASCH-function ra_eval_save_result()", 0);
		if (!eh_use)
			return -1;
	}
	else
	...
	for (i = 0; i < pli->num_create_class; i++)
	{
		...
		n = ra_value_get_num_elem(res[pli->create_class[i].start_pos_idx+res_offset]);
	...
	{
		ret = create_auto_prop(eh, clh, pli->create_class, pli->create_prop+i, res, res_offset);
		if (ret != 0)
			goto clean;
	}
...
int
create_auto_class(eval_handle eh, struct ra_auto_create_class *acc, value_handle *res, long res_offset)
{
	int ret = 0;
...
int
add_events(eval_handle eh, struct ra_auto_create_class *acc, value_handle *res, long res_offset)
{
	int ret = 0;
	long l, num;
...
int
create_auto_prop(eval_handle eh, class_handle clh, struct ra_auto_create_class *acc,
		 struct ra_auto_create_prop *acp, value_handle *res, long res_offset)
{
	...
	if (acp->event_id_idx >= 0)
	{
		...
		num_events = ra_value_get_num_elem(res[acp->event_id_idx+res_offset]);
	}
	else
	{
		...
		{
			/* no, so add them now */
			num_events = ra_value_get_num_elem(res[acc[acp->class_index].start_pos_idx+res_offset]);
			acc[acp->class_index].event_ids = ra_alloc_mem(sizeof(long) * num_events);
			ret = add_events(eh, acc, res, res_offset);
			ev_ids = acc[acp->class_index].event_ids;			
		}
	}
...
int
do_mass_insert(struct ra_auto_create_prop *acp, const long *ev_ids, long num_events, value_handle *res, long res_offset)
{
	int ret;
	long l, m, num;
	long *ch_nums;
	...
	if ((num % num_events) != 0)
	{
		fprintf(stderr, "do_mass_insert() in eval_auto.c: number of values does not match number of events!!!\n");
	...
	if (acp->ch_id >= 0)
	{
		const long *ch = (long *)ra_value_get_long_array(res[acp->ch_id+res_offset]);
		for (l = 0; l < num; l++)
			ch_nums[l] = ch[l];
	}
	else
	...
	ret = ra_prop_set_value_mass(acp->ph, ev_ids_use, ch_nums, res[acp->value_id+res_offset]);
