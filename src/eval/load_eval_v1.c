...
read_event_infos (struct eval_class *cl, xmlNodePtr parent)
{
  ...
  decode_base64 (c, strlen ((const char *) c), &id, &size,
		 sizeof (long) * cl->num_event);
  xmlFree (c);

  node = get_node (parent, "start-pos");
  c = xmlNodeGetContent (node);
  decode_base64 (c, strlen ((const char *) c), &start, &size,
		 sizeof (long) * cl->num_event);
  xmlFree (c);

  node = get_node (parent, "end-pos");
  c = xmlNodeGetContent (node);
  decode_base64 (c, strlen ((const char *) c), &end, &size,
		 sizeof (long) * cl->num_event);
  xmlFree (c);
...
read_events (prop_handle ph, long value_type, xmlNodePtr parent)
{
  ...
  c = xmlNodeGetContent (node);
  decode_base64 (c, strlen ((const char *) c), &ch_map, &size,
		 sizeof (short) * prop->num_ch_values);
  xmlFree (c);
  free (prop->ch_map);
  ...
  switch (prop->num_type)
    ...
    case RA_PROP_NUM_ELEM_TYPE_BYTE:
      free (prop->num_elements.b);
      break;
    case RA_PROP_NUM_ELEM_TYPE_LONG:
      free (prop->num_elements.l);
      break;
    }
  if (prop->num_type == RA_PROP_NUM_ELEM_TYPE_BYTE)
    {
      decode_base64 (c, strlen ((const char *) c), &num_elem, &size,
		     sizeof (unsigned char) * num);
      xmlFree (c);
...
set_char_events (struct eval_property *prop, value_handle vh)
{
  ...
  for (l = 0; l < prop->evclass->num_event; l++)
      ...
      for (m = 0; m < prop->num_ch_values; m++)
	{
	  ...
	  prop->value.c[offset] =
	    malloc (sizeof (char) * strlen (values[curr]) + 1);
...
read_summaries (struct eval_class *cl, xmlNodePtr parent)
{
  ...
  while (node)
      ...
      if (!c)
	{
      ...
      sh =
	ra_sum_add (cl, (const char *) c, NULL, NULL,
		    ra_value_get_num_elem (vh_dim_name),
...
int
read_sum_dimensions (xmlNodePtr node, value_handle vh_dim_name,
		     value_handle vh_dim_unit)
{
  int ret = -1;
  long l, num_dim, idx;
  xmlChar *c;
...
read_summary_ch_desc (sum_handle sh, xmlNodePtr parent)
{
  ...
  while (node)
      ...
      if (xmlStrcmp (node->name, (xmlChar *) "channel-description") != 0)
	{
      ...
      s->num_ch++;
      s->ch_desc =
	realloc (s->ch_desc, sizeof (struct eval_sum_ch_desc) * s->num_ch);
      memset (s->ch_desc + s->num_ch - 1, 0,
...
read_summary_data_element (sum_handle sh, xmlNodePtr parent)
{
  ...
  while (node)
	...
	s->last_data_id = d->id;
      ...
      if ((node_id == NULL) || (node_id->children == NULL)
	  || (node_id->children->next != NULL))
	{
	  _ra_set_error (sh, RA_ERR_EVAL_FILE_CORRUPT, "libRASCH");
	  return -1;
	}
...
read_value (value_handle vh, unsigned long *idx, xmlNodePtr node)
{
  ...
  switch (value_type)
    ...
    case RA_VALUE_TYPE_LONG:
      ...
      break;
    case RA_VALUE_TYPE_DOUBLE:
      decode_base64 (c, strlen ((const char *) c), &t, &size,
		     sizeof (double));
	...
	{
	  fprintf (stderr,
		   "read-value - char-array: cnt(=%ld) != num_elem(%ld)\n",
		   cnt, num_elem);
	  /* TODO: set error code */
	  ret = -1;
	}
