...
write_events(struct eval_property *prop, xmlNodePtr node)
{
	...
	encode_base64(prop->ch_map, sizeof(unsigned short) * prop->num_ch_values, &t, &size);
	t = realloc(t, size+1);
	t[size] = '\0';
	...
	if (prop->num_type == RA_PROP_NUM_ELEM_TYPE_BYTE)
	{
		encode_base64(prop->num_elements.b,
			      sizeof(unsigned char) * prop->num_ch_values * prop->evclass->num_event,
			      &t, &size);
		for (l = 0; l < (size_t)(prop->num_ch_values * prop->evclass->num_event); l++)
			num_data_complete += prop->num_elements.b[l];
	}
	else
	{
		encode_base64(prop->num_elements.l,
			      sizeof(long) * prop->num_ch_values * prop->evclass->num_event,
			      &t, &size);
		for (l = 0; l < (size_t)(prop->num_ch_values * prop->evclass->num_event); l++)
...
int
write_char_events(struct eval_property *prop, xmlNodePtr node, long num_data_complete)
{
	char **values = NULL;
...
int
write_short_array_events(struct eval_property *prop, xmlNodePtr node, long num_data_complete)
{
	short *values = NULL;
	...
	for (l = 0; l < prop->evclass->num_event; l++)
	{
		for (m = 0; m < prop->num_ch_values; m++)
		{
			...
			{
			case RA_PROP_NUM_ELEM_TYPE_BYTE: num = prop->num_elements.b[offset]; break;
			case RA_PROP_NUM_ELEM_TYPE_LONG: num = prop->num_elements.l[offset]; break;
			}
			for (n = 0; n < num; n++)
...
int
write_long_array_events(struct eval_property *prop, xmlNodePtr node, long num_data_complete)
{
	long *values = NULL;
	unsigned long l, m, n, curr, num, offset;
	value_handle vh;
...
int
write_double_array_events(struct eval_property *prop, xmlNodePtr node, long num_data_complete)
{
	double *values = NULL;
	unsigned long l, m, n, curr, num, offset;
	value_handle vh;
...
int
write_char_array_events(struct eval_property *prop, xmlNodePtr node, long num_data_complete)
{
	char **values = NULL;
	unsigned long l, m, n, curr, num, offset;
	value_handle vh;
...
write_summaries(struct eval_class *cl, xmlNodePtr parent)
{
	...
	while (s)
	{
		...
		for (l = 0; l < s->num_dim; l++)
		{
			...
			xmlSetProp(dim_node, (xmlChar *)"name", (xmlChar *)(s->dim_name[l]));
			xmlSetProp(dim_node, (xmlChar *)"unit", (xmlChar *)(s->dim_unit[l]));
		}
...
int
write_attributes(any_handle h, xmlNodePtr parent, const char *modify_timestamp)
{
	int ret = 0;
...
write_value(value_handle vh, long idx, xmlNodePtr parent)
{
	...
	{
	case RA_VALUE_TYPE_SHORT: strcpy(st, "short"); break;
	case RA_VALUE_TYPE_LONG: strcpy(st, "long"); break;
	case RA_VALUE_TYPE_DOUBLE: strcpy(st, "double"); break;
	case RA_VALUE_TYPE_CHAR: strcpy(st, "char"); break;
	case RA_VALUE_TYPE_VOIDP: strcpy(st, "voidp"); break;
	case RA_VALUE_TYPE_SHORT_ARRAY: strcpy(st, "short-array"); break;
	case RA_VALUE_TYPE_LONG_ARRAY: strcpy(st, "long-array"); break;
	case RA_VALUE_TYPE_DOUBLE_ARRAY: strcpy(st, "double-array"); break;
	case RA_VALUE_TYPE_CHAR_ARRAY: strcpy(st, "char-array"); break;
	case RA_VALUE_TYPE_VOIDP_ARRAY: strcpy(st, "voidp-array"); break;
	default:
		/* TODO: set error code */
		return -1;
		break;
	...
	case RA_VALUE_TYPE_VOIDP:
		...
		break;
	case RA_VALUE_TYPE_SHORT_ARRAY:
		encode_base64(ra_value_get_short_array(vh), (sizeof(short) * num), &t, &size);
		break;
	case RA_VALUE_TYPE_LONG_ARRAY:
		encode_base64(ra_value_get_long_array(vh), (sizeof(long) * num), &t, &size);
		break;
	case RA_VALUE_TYPE_DOUBLE_ARRAY:
		encode_base64(ra_value_get_double_array(vh), (sizeof(double) * num), &t, &size);
		break;
	case RA_VALUE_TYPE_CHAR_ARRAY:
		size = 0;
	...
	case RA_VALUE_TYPE_VOIDP_ARRAY:
		encode_base64(ra_value_get_voidp_array(vh), (sizeof(void *) * num), &t, &size);
		break;
	}	
