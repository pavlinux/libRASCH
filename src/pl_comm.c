 ...
 */
LIBRAAPI int
ra_comm_add(meas_handle mh, plugin_handle p, dest_handle dh, const char *sig_name)
{
 ...
 */
LIBRAAPI void
ra_comm_emit(meas_handle mh, const char *sender, const char *sig_name, int num_para, struct comm_para *para)
{
	int i;
	struct ra_meas *mm = (struct ra_meas *) mh;
	struct signal *sig = find_signal(&(mm->plc), sig_name);
	...
	{
		struct plugin_struct *p = (struct plugin_struct *) (sig->slot[i].plugin);
		if (!p)
			continue;
		...
		if (p->handle_signal)
		{
			...
			(*p->handle_signal) (mh, sig->slot[i].dest, sig_name, num_para, para);
		}
	}
	level--;
 ...
 */
void
_trace_signame(int level, const char *sender, const char *sig_name, int num_para, struct comm_para *para)
{
	int i;
	char *sender_locale;
