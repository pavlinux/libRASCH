...
int filter_powerline_noise (double *data, size_t num, double samplerate,
			    value_handle task_data);
 ...
 */
LIBRAAPI int
ra_raw_process (long task, value_handle task_data, size_t num_data,
		DWORD * data, DWORD * data_high, rec_handle rh,
		unsigned int ch)
{
  int ret = 0;
  long l;
  ...
  switch (task)
    ...
    case RA_RAW_PROC_RM_POWERLINE_NOISE:
      ret =
	filter_powerline_noise (data_d, num_data, _ra_get_samplerate (rh, ch),
				task_data);
      break;
...
LIBRAAPI int
ra_raw_process_unit (long task, value_handle task_data, size_t num_data,
		     double *data, rec_handle rh, unsigned int ch)
{
  int ret = 0;
  ...
  switch (task)
    ...
    case RA_RAW_PROC_RM_POWERLINE_NOISE:
      ret =
	filter_powerline_noise (data, num_data, _ra_get_samplerate (rh, ch),
				task_data);
      break;
    case RA_RAW_PROC_RM_MEAN:
...
int
filter_powerline_noise (double *data, size_t num, double samplerate,
			value_handle task_data)
{
  ...
  if (in_linear_period)
    {
      if ((end_idx == 0) || ((end_idx - start_idx) < (l - start_idx_temp)))
	{
	  start_idx = start_idx_temp;
	  end_idx = l;
	}
  ...
  for (l = start_idx; l < end_idx; l++, mean += data[l]);
  ...
  for (phase_test = 0; phase_test < T; phase_test += 0.3)
	...
	{
	  re_t =
	    cos (2 * RA_PI * w0 * ((double) (l - start_idx) + phase_test));
	  im_t =
	    sin (2 * RA_PI * w0 * ((double) (l - start_idx) + phase_test));
	  re += ((data[l] - mean) * re_t);
	  im += ((data[l] - mean) * im_t);
	}
      v = re;
